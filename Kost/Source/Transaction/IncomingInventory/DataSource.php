<?php
	...
	$where = " 1=1 ";
	$order_by = "IncomingInventoryID DESC";
	$rows = 10;
	$current = 1;
	...
	if (ISSET($_REQUEST['searchPhrase']) )
	{
		...
		$where .= " AND ( DATE_FORMAT(II.TransactionDate, '%d-%m-%Y') LIKE '%".$search."%') ";
	}
	...
	$sql = "SELECT
				...
				SUM(IID.Quantity * IID.Price) Total
			FROM
	...
	while ($row = mysql_fetch_array($result)) {
		...
		$row_array['IncomingInventoryID'] = $row['IncomingInventoryID'];
