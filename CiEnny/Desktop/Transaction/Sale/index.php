...
<html>
	...
	<body>
		...
		?>
		<div id="FormData" title="Tambah Penjualan" class="col-md-12" >
			<form class="col-md-12" id="PostForm" method="POST" action="" >
				...
				<div class="row" >
					<h2 style="display: inline-block;float: left;" >ONGKOS &nbsp;: &nbsp;</h2>
					<div style="display: inline-block;top:12px;position:relative;width:20%;" >
						<input id="txtServiceCost" name="txtServiceCost" type="text" class="form-control-custom text-right mousetrap col-md-4" value="0" autocomplete=off onkeypress="return isNumberKey(event, this.id, this.value);" onfocus="clearFormat(this.id, this.value);this.select();" onchange="Calculate();" onblur="convertRupiah(this.id, this.value);" onpaste="return false;" />
					</div>
				</div>
				<div class="row" >
		...
		<script>
			...
			function Calculate() {
				var grandTotal = 0;
				var weight = 0;
				var serviceCost = parseFloat($("#txtServiceCost").val().replace(/\,/g, ""));
				...
				grandTotal = grandTotal + serviceCost;
			...
			function printInvoice() {
				...
				if(PassValidate == 1) {
					...
					var TransactionDate = $("#hdnTransactionDate").val();
					var ServiceCost = $("#txtServiceCost").val().replace(/\,/g, "");
					$("#loading").show();
					$.ajax({
						url: "./Transaction/Sale/PrintInvoice.php",
						type: "POST",
						data: { SaleID : saleID, Payment : Payment, PaymentType : PaymentType, PrintInvoice : PrintInvoice, Change: Change, SaleNumber : SaleNumber, PaymentMethod : PaymentMethod, TransactionDate : TransactionDate, ServiceCost : ServiceCost },
						dataType: "json",
						success: function(data) {
							if(data.FailedFlag == '0') {
								$("#loading").hide();
			...
			$(document).ready(function() {
				...
				var counterKey = 0;
				$(document).on("keydown", function (evt) {
					if(evt.keyCode == 123 && $("#itemList-dialog").css("display") == "none" && $("#finish-dialog").css("display") == "none" && $("#code-dialog").css("display") == "none" && $("#add-confirm").css("display") == "none" && $(".lobibox").css("display") != "block"  && $("#save-confirm").css("display") == "none" && $("#delete-confirm").css("display") == "none" ) {
						evt.preventDefault();
						if(counterKey == 0) {
							itemList();
							counterKey = 1;
					...
					else if(evt.keyCode == 121 && $("#itemList-dialog").css("display") == "none"  && $("#finish-dialog").css("display") == "none" && $("#code-dialog").css("display") == "none" && $("#add-confirm").css("display") == "none" && $(".lobibox").css("display") != "block" && $("#save-confirm").css("display") == "none" && $("#delete-confirm").css("display") == "none" ) {
						evt.preventDefault();
						if(counterKey == 0) {
							finish();
							counterKey = 1;
