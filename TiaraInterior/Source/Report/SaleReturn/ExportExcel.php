<?php
	if(ISSET($_GET['CustomerID'])) {
		...
		$objPHPExcel->getProperties()->setCreator($_SESSION['UserLogin'])
									 ...
									 ->setTitle("Laporan Retur Jual")
									 ->setSubject("Laporan")
									 ->setDescription("Laporan Retur Jual")
									 ->setKeywords("Generate By PHPExcel")
									 ->setCategory("Laporan");
		//Header
		$objPHPExcel->setActiveSheetIndex(0)
					->setCellValue('A1', "LAPORAN RETUR JUAL");
		...
		while($row = mysql_fetch_array($result)) {
			...
			$objPHPExcel->getActiveSheet()->setCellValue("B".$rowExcel, $row['SaleReturnNumber']);
			...
			$objPHPExcel->getActiveSheet()->setCellValue("G".$rowExcel, $row['SalePrice']);
		...
		$title = "Laporan Retur Jual $FromDate - $ToDate";
		// Rename worksheet
