		...
		if($ProjectID != 0) {
			...
			if($rowCount > 0) {
				...
				while($row = mysql_fetch_array($result)) {
					...
					array_push($Data, "'".$row['ProjectTransactionID']."', '".$row['ProjectTransactionDate']."', '".$row['Remarks']."', '".$row['Amount']."'");
				}
	...
	<body>
									...
									<table class="table" id="datainput">
										...
										<tbody>
											<tr id='' style='display:none;' class="num">
												...
												<td>
													<input type="hidden" id="hdnProjectTransactionID" class="hdnProjectTransactionID" name="hdnProjectTransactionID" value="0" />
											...
											<tr id="num1" name="num1" class="num">
												...
												<td>
													<input type="hidden" id="hdnProjectTransactionID1" class="hdnProjectTransactionID" name="hdnProjectTransactionID1" value="0" />
						...
						<div class="row">
							<div class="col-md-10">
								<div class="col-md-4">
									...
									<button class="btn btn-default" id="btnSave"  onclick="SubmitForm('./Transaction/ProjectOperational/Insert.php');" ><i class="fa fa-save "></i> Simpan</button>
								</div>
							</div>
						</div>
					</div>
		...
		<script>
			...
			function RegenerateRowNumber() {
				...
				i = 0;
				$(".hdnProjectTransactionID").each(function() {
					if(i != 0) {
						$(this).attr("id", "hdnProjectTransactionID" + i);
						$(this).attr("name", "hdnProjectTransactionID" + i);
					}
					i++;
				});
				i = 0;
			...
			$(document).ready(function () {
				...
				if(parseInt($("#hdnRow").val()) > 0) {
					...
					for(var i=0; i<row; i++) {
						...
						var d = item[i].split("', '");
						$("#nota").text(count);
						$("#hdnProjectTransactionID" + count).val(d[0].replace("'", ""));
