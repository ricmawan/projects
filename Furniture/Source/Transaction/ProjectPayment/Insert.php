	...
	if(isset($_POST['hdnProjectID'])) {
		...
		if($RecordNew > 0) {
			for($i=1;$i<=$RecordNew;$i++) {
				$DetailID .= $_POST['hdnProjectPaymentID'.$i].",";
			}
		...
		$sql = "DELETE 
				...
				WHERE
					ProjectPaymentID NOT IN($DetailID)			 
					AND ProjectID = $ID";
		...
		if($RecordNew > 0) {
			for($j=1;$j<=$RecordNew;$j++) {
				...
				if($_POST['hdnProjectPaymentID'.$j] == "0") {
					$State = 4;
					$sql = "INSERT INTO transaction_projectpayment
							(
								ProjectID,
					...
					$sql = "UPDATE 
							...
							WHERE
								ProjectPaymentID = ".$_POST['hdnProjectPaymentID'.$j];
				}
