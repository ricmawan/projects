	...
	if(isset($_POST['ID'])) {
		...
		for($i=0; $i<count($Data); $i++) {
			try
			{
				...
				$CommonOperationalID = mysql_real_escape_string($Data[$i]);
				$sql = "DELETE FROM transaction_commonoperational WHERE CommonOperationalID = $CommonOperationalID";
				if (! $result=mysql_query($sql, $dbh)) {
					throw new Exception($ProjectName);
				}
				$MessageSuccessDelete .= "$CommonOperationalID, ";
			}
			catch (Exception $e)
			{
