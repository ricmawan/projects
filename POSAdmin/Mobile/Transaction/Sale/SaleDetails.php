<?php
	if(ISSET($_POST['SaleID'])) {
		...
		if (! $result = mysqli_query($dbh, $sql)) {
			logEvent(mysqli_error($dbh), '/Transaction/Sale/SaleDetails.php', mysqli_real_escape_string($dbh, $_SESSION['UserLoginMobile']));
			$FailedFlag = 1;
			$json_data = array(
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			$row_array[] = "<div id='toggle-branch-" . $row['SaleDetailsID'] . "' onclick=\"updateBranch(this.id, " . $row['Quantity'] . ", '". $row['ItemCode'] ."')\" class='div-center toggle-modern' ></div>";
