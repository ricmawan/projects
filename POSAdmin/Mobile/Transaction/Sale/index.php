	...
	<body>
		...
		<script>
			...
			function openDialogEdit(Data) {
				...
				$("#txtQTY").val(Data[7]);
				$("#txtSalePrice").val(Data[8].replace(/\,/g, ""));
				$("#txtDiscount").val(Data[9].replace(/\,/g, ""));
								...
								if(data.FailedFlag == '0') {
									if($("#hdnSaleDetailsID").val() == 0) {
										...
										table2.row.add([
											...
											itemName,
											Qty,
											returnRupiah(salePrice.toString()),
			...
			function newWindow() {
				window.open("http://192.168.1.21/Projects/POSAdmin/Mobile/Home.php", "", "width=1000");
			...
			function Change() {
				...
				if(parseFloat(Payment) == 0) {
					...
					if(parseFloat(Total) > parseFloat(Payment)) {
						...
						$("#loading").show();
						$.ajax({
							url: "./Transaction/Sale/UpdatePayment.php",
							type: "POST",
							data: { SaleID : saleID, Payment : Payment },
							dataType: "json",
							success: function(data) {
								if(data.FailedFlag == '0') {
									$("#loading").hide();
