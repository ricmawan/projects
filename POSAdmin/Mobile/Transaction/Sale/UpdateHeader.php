<?php
	if(ISSET($_POST['SaleID']) && ISSET($_POST['CustomerID'])) {
		...
		$sql = "CALL spUpdSale(".$SaleID.", ".$CustomerID.", '".$TransactionDate."', '".$_SESSION['UserLoginMobile']."')";
		if (! $result=mysqli_query($dbh, $sql)) {
			$Message = "Terjadi Kesalahan Sistem";
			$MessageDetail = mysqli_error($dbh);
			$FailedFlag = 1;
			logEvent(mysqli_error($dbh), '/Transaction/Sale/UpdateHeader.php', mysqli_real_escape_string($dbh, $_SESSION['UserLoginMobile']));
