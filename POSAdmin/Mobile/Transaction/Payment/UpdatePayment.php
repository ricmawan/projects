<?php
	if(ISSET($_POST['TransactionID']) && ISSET($_POST['Payment'])) {
		...
		$sql = "CALL spUpdPayment(".$TransactionID.", ".$Payment.", '".$TransactionType."', '".$_SESSION['UserLoginMobile']."')";
		if (! $result=mysqli_query($dbh, $sql)) {
			$Message = "Terjadi Kesalahan Sistem";
			$MessageDetail = mysqli_error($dbh);
			$FailedFlag = 1;
			logEvent(mysqli_error($dbh), '/Transaction/Payment/UpdatePayment.php', mysqli_real_escape_string($dbh, $_SESSION['UserLoginMobile']));
