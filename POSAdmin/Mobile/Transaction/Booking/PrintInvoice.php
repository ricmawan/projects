<?php
	...
	if(ISSET($_POST['BookingID']) && ISSET($_POST['Payment'])) {
		...
		$sql = "CALL spUpdBookingPayment(".$BookingID.", ".$Payment.", ".$PaymentType.", ".$FinishFlag.", ".$DiscountTotal.", '".$_SESSION['UserLoginMobile']."')";
		if (! $result=mysqli_query($dbh, $sql)) {
			$Message = "Terjadi Kesalahan Sistem";
			$MessageDetail = mysqli_error($dbh);
			$FailedFlag = 1;
			logEvent(mysqli_error($dbh), '/Transaction/Booking/UpdatePayment.php', mysqli_real_escape_string($dbh, $_SESSION['UserLoginMobile']));
		...
		if($IsAttached == 'Y') {
			...
			if($PrintInvoice == "true") {
			    ...
			    $sql = "CALL spSelBookingHeader(".$BookingID.", '".$_SESSION['UserLoginMobile']."')";

				if (! $result = mysqli_query($dbh, $sql)) {
					logEvent(mysqli_error($dbh), '/Transaction/Booking/PrintInvoice.php', mysqli_real_escape_string($dbh, $_SESSION['UserLoginMobile']));
					$Message = "Terjadi Kesalahan Sistem";
			    ...
			    while ($row = mysqli_fetch_array($result)) {
			        ...
			        $rowPrice .= $Quantity . " " . $row['UnitName'] . " @ " . number_format($row['BookingPrice'] - $row['Discount'],0,".",",");
