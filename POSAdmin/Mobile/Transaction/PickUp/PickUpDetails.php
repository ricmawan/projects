<?php
	if(ISSET($_POST['PickUpID'])) {
		...
		$sql = "CALL spSelPickUpDetails(".$PickUpID.", '".$_SESSION['UserLoginMobile']."')";
		$FailedFlag = 0;

		if (! $result = mysqli_query($dbh, $sql)) {
			logEvent(mysqli_error($dbh), '/Transaction/PickUp/PickUpDetails.php', mysqli_real_escape_string($dbh, $_SESSION['UserLoginMobile']));
			$FailedFlag = 1;
			$json_data = array(
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			$row_array[] = "<input type='checkbox' name='chkBookingDetails[]' class='chkBookingDetails' value=" . $row['PickDetailsID'] . " onclick='Calculate();' tabindex=".$tabindex." />";
			...
			$row_array[] = '<input id="txtQty'. $row['PickDetailsID'] .'" name="txtQty'. $row['PickDetailsID'] .'" type="number" class="form-control-custom text-right txtQTY" style="height:20px !important;width: 100%;margin: 0;border: 0;padding-right: 22px;" value='.$row['Quantity'].' max='.$row['Maksimum'].' min=1 onfocus="this.select();" autocomplete=off placeholder="Qty Grosir 1" onpaste="return false;" onchange="validateQTY2(this)" required tabindex='.($tabindex+1).' />';
