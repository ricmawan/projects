<?php
	if(ISSET($_POST['BookingNumber'])) {
		...
		$sql = "CALL spSelBookingDetailsByNumber('".$BookingNumber."', '".$_SESSION['UserLoginMobile']."')";
		$FailedFlag = 0;

		if (! $result = mysqli_query($dbh, $sql)) {
			logEvent(mysqli_error($dbh), '/Transaction/PickUp/BookingDetails.php', mysqli_real_escape_string($dbh, $_SESSION['UserLoginMobile']));
			$FailedFlag = 1;
			$json_data = array(
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			$row_array[] = "<input type='checkbox' name='chkBookingDetails[]' class='chkBookingDetails' value=" . $row['BookingDetailsID'] . " onclick='Calculate();'".$disabled." tabindex=".$tabindex." />";
			$row_array[] = "<div id='toggle-branch-" . $row['BookingDetailsID'] . "' class='div-center toggle-modern' onclick='updateBranch(this.id)' ></div>
				...
				<input id='hdnBookingPrice". $row['BookingDetailsID'] ."' name='hdnBookingPrice". $row['BookingDetailsID'] ."' type='hidden' value=".$row['BookingPrice'] / $row['ConversionQuantity']." />
				<input id='hdnDiscount". $row['BookingDetailsID'] ."' name='hdnDiscount". $row['BookingDetailsID'] ."' type='hidden' value=".$row['Discount']." />
			...
			$row_array[] = '<input id="txtQty'. $row['BookingDetailsID'] .'" name="txtQty'. $row['BookingDetailsID'] .'" type="number" class="form-control-custom text-right txtQTY" style="height:20px !important;width: 100%;margin: 0;border: 0;padding-right: 22px;" value='.$row['Quantity'].' max='.$row['Quantity'].' min=1 onfocus="this.select();" autocomplete=off placeholder="Qty Grosir 1" onpaste="return false;" onchange="validateQTY2(this)" '.$disabled.' tabindex='.($tabindex+1).' />';
