<?php
	...
	try {
		if(ISSET($_SESSION['UserLoginMobile']) && ISSET($_SESSION['UserPassword'])) {
			...
			if (!$result = mysqli_query($dbh, $sql)) {
				logEvent(mysqli_error($dbh), '/index.php', mysqli_real_escape_string($_SESSION['UserLoginMobile']));
				return 0;
			}
			...
			if($cek == 1) {
				header("Location: ".$MOBILE_PATH."Home.php", true, 301);
				return;
			}
		}
	}
	...
	{
		logEvent($e->getMessage(), '/index.php', mysqli_real_escape_string($dbh, $_SESSION['UserLoginMobile']));
	}
?>
