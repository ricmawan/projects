<?php
	if(ISSET($_POST['BookingID'])) {
		...
		if (! $result = mysqli_query($dbh, $sql)) {
			logEvent(mysqli_error($dbh), '/Transaction/Booking/BookingDetails.php', mysqli_real_escape_string($dbh, $_SESSION['UserLogin']));
			$FailedFlag = 1;
			$json_data = array(
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			$row_array[] = "<div id='toggle-branch-" . $row['BookingDetailsID'] . "' onclick='updateBranch(this.id)' class='div-center toggle-modern' ></div>";
