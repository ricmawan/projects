<?php
	if(ISSET($_POST['SaleReturnID'])) {
		...
		$sql = "CALL spSelSaleReturnDetails(".$SaleReturnID.", '".$_SESSION['UserLoginKasir']."')";
		$FailedFlag = 0;

		if (! $result = mysqli_query($dbh, $sql)) {
			logEvent(mysqli_error($dbh), '/Transaction/SaleReturn/SaleReturnDetails.php', mysqli_real_escape_string($dbh, $_SESSION['UserLoginKasir']));
			$FailedFlag = 1;
			$json_data = array(
