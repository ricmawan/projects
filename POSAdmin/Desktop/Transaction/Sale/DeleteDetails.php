	...
	if(isset($_POST['ID'])) {
		...
		for($i=0; $i<count($ArrayData); $i++) {
			try
			{
				...
				$sql = "CALL spDelSaleDetails($DataID, '".$_SESSION['UserLoginKasir']."')";
				if (!$result = mysqli_query($dbh, $sql)) {
					logEvent(mysqli_error($dbh), '/Transaction/Sale/Delete.php', mysqli_real_escape_string($dbh, $_SESSION['UserLoginKasir']));
