<?php
	...
	if(ISSET($requestData['BranchID']) && ISSET($requestData['CategoryID']) && ISSET($requestData['FirstPass']) && $requestData['FirstPass'] == "0")
	{
		...
		$sql = "CALL spSelNoStockReport(".$CategoryID.", ".$BranchID.", \"$where\", '$order_by', $limit_s, $limit_l, '".$_SESSION['UserLogin']."')";

		if (! $result = mysqli_query($dbh, $sql)) {
			logEvent(mysqli_error($dbh), '/Report/NoStock/DataSource.php', mysqli_real_escape_string($dbh, $_SESSION['UserLogin']));
			return 0;
		}
