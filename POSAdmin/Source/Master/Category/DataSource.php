<?php
	...
	if (!empty($requestData['search']['value']))
	{
		...
		$where .= " AND ( MC.CategoryCode LIKE '%".$search."%'";
		$where .= " OR MC.CategoryName LIKE '%".$search."%' )";
	}
	$sql = "CALL spSelCategory(\"$where\", '$order_by', $limit_s, $limit_l, '".$_SESSION['UserLogin']."')";

	if (! $result = mysqli_query($dbh, $sql)) {
		logEvent(mysqli_error($dbh), '/Master/Category/DataSource.php', mysqli_real_escape_string($dbh, $_SESSION['UserLogin']));
	...
	while ($row = mysqli_fetch_array($result2)) {
		...
		$row_array[] = "<input name='select' type='checkbox' value='".$row['CategoryID']."^".$row['CategoryName']."' />";
		$row_array[] = $RowNumber;
		$row_array[] = $row['CategoryCode'];
