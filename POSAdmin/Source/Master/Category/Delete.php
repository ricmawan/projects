	...
	if(isset($_POST['ID'])) {
		...
		for($i=0; $i<count($ArrayData); $i++) {
			try
			{
				...
				$DataName = $Data[1];
				$sql = "CALL spDelCategory($DataID, '".$_SESSION['UserLogin']."')";
				if (!$result = mysqli_query($dbh, $sql)) {
					logEvent(mysqli_error($dbh), '/Master/Category/Delete.php', mysqli_real_escape_string($dbh, $_SESSION['UserLogin']));
		...
		if($MessageSuccessDelete !="") $MessageSuccess = "Kategori " .$MessageSuccessDelete. " Berhasil Dihapus";
		else $MessageSuccess = "";
		if($MessageFailedDelete !="") $MessageFailed = "Kategori " .$MessageFailedDelete. " Gagal Dihapus";
		else $MessageFailed = "";
