	...
	<body>
		...
		<div id="FormData" title="Tambah Penjualan" style="display: none;">
			<form class="col-md-12" id="PostForm" method="POST" action="" >
				...
				<br />
				<div class="row" >
					<h5 style="margin: 5px 0 0 0;">F1= Batalkan Transaksi;F2= Hapus Barang Terakhir;F8= Cetak Nota Terakhir;F10= Transaksi Selesai;F11= List Pelanggan;F12= List Barang;ESC= Tutup;DELETE= Hapus;ENTER/DOUBLE KLIK= Edit;</h5>
				</div>
			</form>
		</div>
		...
		<script>
				...
				$.ajax({
					...
					success: function(Data) {
						if(Data.FailedFlag == '0') {
							...
							//table2.draw();
							tableWidthAdjust("getsaledetails");
			...
			function addSaleDetailsRow() {
				...
				var subTotal = returnRupiah(((parseFloat(salePrice.replace(/\,/g, "")) - parseFloat(discount.replace(/\,/g, "")) ) * parseFloat(Qty)).toString());
				//discount = returnRupiah(discount.toString());
				...
				$.ajax({
					...
					success: function(data) {
						if(data.FailedFlag == '0') {
							...
							tableWidthAdjust("add saledetails row");
							Calculate();
						}
						else {
							var counter = 0;
			...
			function tableWidthAdjust(caller) {
				...
				//console.log($("#divTableContent").find("table"));
				//alert(caller);
			}
					...
					if(counterDeleteDetails == 0) {
						...
						SingleDelete("./Transaction/Sale/DeleteDetails.php", deletedData, function(action) {
							if(action == "success") {
								...
								tableWidthAdjust("DeleteLastDetails");
								Calculate();
							}
							else {
								table2.keys.enable();
			...
			$(document).ready(function() {
					...
					waitForFinalEvent(function () {
		               	setTimeout(function() {
							...
							if ( $.fn.DataTable.isDataTable( '#grid-transaction' ) ) {
								tableWidthAdjust("window resize");
							}
				...
				$(document).on("keydown", function (evt) {
					...
					else if(evt.keyCode == 123 && $("#customerList-dialog").css("display") == "none" && $("#itemList-dialog").css("display") == "none" && $("#finish-dialog").css("display") == "none" && $("#code-dialog").css("display") == "none" && $("#add-confirm").css("display") == "none" && $("#FormData").css("display") == "block"  && $(".lobibox").css("display") != "block"  && $("#save-confirm").css("display") == "none" && $("#delete-confirm").css("display") == "none" ) {
						evt.preventDefault();
						if(counterKey == 0) {
							itemList();
							counterKey = 1;
					...
					else if(evt.keyCode == 121 && $("#customerList-dialog").css("display") == "none" && $("#itemList-dialog").css("display") == "none"  && $("#finish-dialog").css("display") == "none" && $("#code-dialog").css("display") == "none" && $("#add-confirm").css("display") == "none" && $("#FormData").css("display") == "block"  && $(".lobibox").css("display") != "block" && $("#save-confirm").css("display") == "none" && $("#delete-confirm").css("display") == "none" ) {
						evt.preventDefault();
						if(counterKey == 0) {
							finish();
							counterKey = 1;
