		...
		if($cek==0) {
			...
			for($i=0; $i<count($data); $i++) {
				try
				{
					...
					$username = $user[1];
					$iduser = $user[0];
					$sql = "DELETE FROM master_layanan WHERE LayananID = $iduser";
			...
			if($messagesuccessdelete !="") $messagesuccess = "Layanan " .$messagesuccessdelete. " Berhasil Dihapus";
			else $messagesuccess = "";
			if($messagefaileddelete !="") $messagefailed = "Layanan " .$messagefaileddelete. " Gagal Dihapus";
			else $messagefailed = "";
