		...
		if($cek==0) {
			...
			for($i=0; $i<count($data); $i++) {
				try
				{
					...
					$username = $user[1];
					$iduser = $user[0];
					$sql = "DELETE FROM master_jobdesk WHERE JobdeskID = $iduser";
			...
			if($messagesuccessdelete !="") $messagesuccess = "Jobdesk Asisten " .$messagesuccessdelete. " Berhasil Dihapus";
			else $messagesuccess = "";
			if($messagefaileddelete !="") $messagefailed = "Jobdesk Asisten " .$messagefaileddelete. " Gagal Dihapus";
			else $messagefailed = "";
