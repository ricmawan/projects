		...
		if($cek==0) {
			...
			for($i=0; $i<count($data); $i++) {
				try
				{
					...
					$username = $user[1];
					$iduser = $user[0];
					$sql = "DELETE FROM master_asisten WHERE AsistenID = $iduser";
			...
			if($messagesuccessdelete !="") $messagesuccess = "Asisten " .$messagesuccessdelete. " Berhasil Dihapus";
			else $messagesuccess = "";
			if($messagefaileddelete !="") $messagefailed = "Asisten " .$messagefaileddelete. " Gagal Dihapus";
			else $messagefailed = "";
