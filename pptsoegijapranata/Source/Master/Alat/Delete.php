		...
		if($cek==0) {
			...
			for($i=0; $i<count($data); $i++) {
				try
				{
					...
					$username = $user[1];
					$iduser = $user[0];
					$sql = "DELETE FROM master_alat WHERE AlatID = $iduser";
			...
			if($messagesuccessdelete !="") $messagesuccess = "Alat " .$messagesuccessdelete. " Berhasil Dihapus";
			else $messagesuccess = "";
			if($messagefaileddelete !="") $messagefailed = "Alat " .$messagefaileddelete. " Gagal Dihapus";
			else $messagefailed = "";
