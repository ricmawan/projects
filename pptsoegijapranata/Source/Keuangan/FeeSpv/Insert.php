		...
		if($cek==0) {
			...
			for($j=1;$j<=$RecordNew;$j++) {
				if($_POST['hdnDetailID'.$j] == "0") {
					$State = 4;
					$sql = "INSERT INTO transaksi_rincifee
						(
							DetailID,
							TransaksiID,
							Keterangan,
						...
						(
							...
							".str_replace(".", "", str_replace(",00", "", $_POST['txtJumlah'.$j])).",
							'".$_POST['txtInst']."',
							NOW(),
							'".$_SESSION['Username']."'
						)";
					...
					$sql = "UPDATE 
						...
						SET
							...
							Jumlah = ".str_replace(".", "", str_replace(",00", "", $_POST['txtJumlah'.$j])).",
							Instfee = '".$_POST['txtInst']."',
							ModifiedBy = '".$_SESSION['Username']."'
						WHERE
