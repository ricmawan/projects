...
function isNumberKey(evt, id, value) {
	...
	if (charCode > 31 && (charCode < 47 || charCode > 57))
		return false;
	if (e.shiftKey) return false;
	if (charCode == 13) /*convertRupiah(id, value);*/ $("#" + id).blur();
	return true;
}
...
function clearFormat(id, angka) {
	//alert(angka);
	//var angka1 = angka.replace(/,00/g, "");
	//angka1 = angka1.replace(/\./g, "");
...
(function( $ ) {
	$.widget( "custom.combobox", {
		...
		_createAutocomplete: function() {
			...
			this.input = $( "<input style='font-family: Open Sans, sans-serif; font-size: 14px;' placeholder='Pilih Klien'>" )
			...
			.autocomplete({
				...
				select: function(event, ui) {
						...
						/*$.ajax({
							url: "./CustomerService/CustomerService/GetContact.php",
							type: "POST",
							data: { ID : $("#ddlKlien").val() },
							dataType: "html",
							success: function(data) {
								$("#txtCP").val(data);
							},
		...
		_removeIfInvalid: function( event, ui ) {
			...
			.notify("Tidak Cocok Dengan Pilihan Yang Ada!", { position:"right", className:"warn", autoHideDelay: 2000 })
			.tooltip( "open" );
			this.element.val( "" );
