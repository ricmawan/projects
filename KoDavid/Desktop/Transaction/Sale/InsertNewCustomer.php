<?php
	if(isset($_POST['txtCustomerCodeAdd'])) {
		...
		$CustomerPrice = mysqli_real_escape_string($dbh, $_POST['ddlCustomerPrice']);
		$hdnIsEdit = 0;
		...
		if (! $result=mysqli_query($dbh, $sql)) {
			...
			$FailedFlag = 1;
			logEvent(mysqli_error($dbh), '/Transaction/Sale/InsertNewCustomer.php', mysqli_real_escape_string($dbh, $_SESSION['UserLoginKasir']));
