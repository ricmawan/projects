<?php
	if(ISSET($_POST['BookingID']) && ISSET($_POST['CustomerID'])) {
		...
		$sql = "CALL spUpdBooking(".$BookingID.", ".$CustomerID.", '".$TransactionDate."', '".$_SESSION['UserLoginKasir']."')";
		if (! $result=mysqli_query($dbh, $sql)) {
			$Message = "Terjadi Kesalahan Sistem";
			$MessageDetail = mysqli_error($dbh);
			$FailedFlag = 1;
			logEvent(mysqli_error($dbh), '/Transaction/Booking/UpdateHeader.php', mysqli_real_escape_string($dbh, $_SESSION['UserLoginKasir']));
