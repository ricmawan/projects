<?php
	if(isset($_POST['txtSupplierCodeAdd'])) {
		...
		$SupplierID = 0;
		$SupplierCode = mysqli_real_escape_string($dbh, $_POST['txtSupplierCodeAdd']);
		$SupplierName = mysqli_real_escape_string($dbh, $_POST['txtSupplierNameAdd']);
		...
		if (! $result=mysqli_query($dbh, $sql)) {
			...
			$FailedFlag = 1;
			logEvent(mysqli_error($dbh), '/Transaction/Purchase/InsertNewSupplier.php', mysqli_real_escape_string($dbh, $_SESSION['UserLogin']));
			echo returnstate($SupplierID, $Message, $MessageDetail, $FailedFlag, $State);
			return 0;
		}				
		$row=mysqli_fetch_array($result);
