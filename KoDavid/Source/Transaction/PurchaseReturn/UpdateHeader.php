<?php
	if(ISSET($_POST['PurchaseID']) && ISSET($_POST['SupplierID'])) {
		...
		$sql = "CALL spUpdPurchaseReturn(".$PurchaseID.", ".$SupplierID.", '".$TransactionDate."', '".$_SESSION['UserLogin']."')";
		if (! $result=mysqli_query($dbh, $sql)) {
			$Message = "Terjadi Kesalahan Sistem";
			$MessageDetail = mysqli_error($dbh);
			$FailedFlag = 1;
			logEvent(mysqli_error($dbh), '/Transaction/PurchaseReturn/UpdateHeader.php', mysqli_real_escape_string($dbh, $_SESSION['UserLogin']));
