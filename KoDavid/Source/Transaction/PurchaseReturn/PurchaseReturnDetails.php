<?php
	if(ISSET($_POST['PurchaseReturnID'])) {
		header('Content-Type: application/json');
		$RequestedPath = "$_SERVER[REQUEST_URI]";
		$file = basename($RequestedPath);
		$RequestedPath = str_replace($file, "", $RequestedPath);
		include "../../GetPermission.php";
		$PurchaseReturnID = mysqli_real_escape_string($dbh, $_POST['PurchaseReturnID']);
		$sql = "CALL spSelPurchaseReturnDetails(".$PurchaseReturnID.", '".$_SESSION['UserLogin']."')";
		$FailedFlag = 0;

		if (! $result = mysqli_query($dbh, $sql)) {
			logEvent(mysqli_error($dbh), '/Transaction/PurchaseReturn/PurchaseReturnDetails.php', mysqli_real_escape_string($dbh, $_SESSION['UserLogin']));
			$FailedFlag = 1;
			$json_data = array(
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			$row_array[] = $row['PurchaseReturnDetailsID'];
