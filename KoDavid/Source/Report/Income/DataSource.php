<?php
	...
	if(ISSET($requestData['BranchID']) && ISSET($requestData['FirstPass']) && $requestData['FirstPass'] == "0")
	{
		...
		if (!empty($requestData['search']['value']))
		{
			...
			$where3 .= " AND ( TB.BookingNumber LIKE '%".$search."%'";
			$where3 .= " OR DATE_FORMAT(TB.TransactionDate, '%d-%m-%Y') LIKE '%".$search."%'";
			$where3 .= " OR MC.CustomerName LIKE '%".$search."%' )";
		}
		$sql = "CALL spSelIncomeReport(".$BranchID.", '".$txtFromDate."', '".$txtToDate."', \"$where\", \"$where2\", \"$where3\", '$order_by', $limit_s, $limit_l, '".$_SESSION['UserLogin']."')";
		...
		while ($row = mysqli_fetch_array($result2)) {
			...
			$row_array["Profit"] = number_format($row['Profit'],0,".",",");
			$row_array["Discount"] = number_format($row['Discount'],0,".",",");
			$row_array["TotalProfit"] = number_format($row['TotalProfit'],0,".",",");
			$row_array["SaleID"] = $row['SaleID'];
			$row_array["TransactionType"] = $row['TransactionType'];
			$SubTotal += $row['TotalProfit'];
