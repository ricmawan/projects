DROP PROCEDURE IF EXISTS spInsTable;

DELIMITER $$
CREATE PROCEDURE spInsTable (
	pID 			BIGINT, 
	pTableNumber	VARCHAR(100),
	pTableStatusID	SMALLINT,
	pIsEdit			INT,
...
SET State = 2;
			SELECT
				pID AS 'ID',
				'Nomor meja sudah ada' AS 'Message',
				'' AS 'MessageDetail',
				1 AS 'FailedFlag',
				State AS 'State' ;
...
SET State = 8;
		SELECT
			pID AS 'ID',
			'Meja berhasil ditambahkan' AS 'Message',
			'' AS 'MessageDetail',
			0 AS 'FailedFlag',
			State AS 'State';
	ELSE
SET State = 9;
		SELECT
			pID AS 'ID',
			'Meja berhasil diubah' AS 'Message',
			'' AS 'MessageDetail',
			0 AS 'FailedFlag',
			State AS 'State';
	END IF;
