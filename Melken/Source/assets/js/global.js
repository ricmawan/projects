...
$(document).ready(function () {
	...
	$(document).on("click", ".menu", function() {
		...
		if(MenuClicked != "./Home.php") {
			...
			$("#page-inner").html('<img src="./assets/img/logo.png" class="imgLogo" />');
			$("#loading").hide();
		}
...
function convertThousand(id, angka){
	if(angka.indexOf(",") < 0 && angka != "" && angka != ".00") {
		var flag = 0;
		if(angka.indexOf(".") > 0) {
			var temp = angka.split(".");
...
(function( $ ) {
	$.widget( "custom.combobox", {
		...
		_createAutocomplete: function() {
			...
			wasOpen = false;
			this.input = $( "<input style='font-family: Open Sans, sans-serif; font-size: 20px;'>" )
