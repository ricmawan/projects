					...
					<div class="panel-body">
						<form class="col-md-12" id="PostForm" method="POST" action="" >
							...
							<div class="row">
								...
								<div class="col-md-3">
									<select name="ddlUserType" id="ddlUserType" class="form-control-custom" placeholder="Pilih Jabatan" onchange="SetRole();" >
										<option value="0" selected>-Pilih Jabatan-</option>
										<?php
											$sql = "SELECT UserTypeID, UserTypeName FROM master_usertype";
											if(!$result = mysql_query($sql, $dbh)) {
												echo mysql_error();
												return 0;
											}
											while($row = mysql_fetch_array($result)) {
												if($UserTypeID == $row['UserTypeID']) echo "<option selected value='".$row['UserTypeID']."' >".$row['UserTypeName']."</option>";
												else echo "<option value='".$row['UserTypeID']."' >".$row['UserTypeName']."</option>";
											}
										?>
									</select>
								</div>-->
