<?php
	if(isset($_POST['SaleID'])) {
		...
		if($SaleID != 0) {
			...
			if($IsPercentage == true) $Discount = $row['Discount'];
			else $Discount = number_format($row['Discount'], 2,".",",");
			...
			if($rowCount > 0) {
				...
				while($row = mysql_fetch_array($result)) {
					array_push($Data, "'".$row['SaleDetailsID']."', '".$row['MenuListID']."', '".$row['MenuName']."', '".$row['Quantity']."', '".$row['Price']."', '".$row['Discount']."', '".$row['IsPercentage']."'");
				}
				$Data = implode("|", $Data);
			}
			else {
	...
	<body>
						...
						<form class="col-md-12" id="PostForm" method="POST" action="" >
							<div class="row">
								...
								<div class="col-md-4">
									...
									<input id="hdnIsPercentage" name="hdnIsPercentage" type="hidden" <?php echo 'value="'.$IsPercentage.'"'; ?> />
							...
							<div class="row">
								<div class="col-md-12">
									<table class="table" style="width:auto;" id="datainput">
										<thead style="background-color: black;color:white;height:35px;width:1013px;display:block;">
										...
										<tbody style="display:block;max-height:245px;height:100%;overflow-y:auto;">
											<tr id='' style='display:none;' class="num">
												<td id='nota' name='nota' class='nota' style="width:34px;vertical-align:middle;border: solid 1px black;"></td>
												<td style="width:310px;border: solid 1px black;">
												...
												<td style="width:80px;border: solid 1px black;">
												...
												<td style="width:170px;border: solid 1px black;">
												...
												<td style="width:195px;border: solid 1px black;">
												...
												<td  style="width:190px;border: solid 1px black;">
							...
							<div class="row">
								...
								<div class="col-md-3">
									<input type="text" id="txtDiscountTotal" <?php echo 'value="'.$Discount.'"'; ?> value="0" onClick="this.select();" style="text-align:right;" name="txtDiscountTotal" class="form-control-custom" onchange="ValidateDiscountTotal();" onkeypress="return isNumberKey(event, this.id, this.value)" onfocus="clearFormat(this.id, this.value)" /> &nbsp;
								</div>
								<div class="col-md-1">
		...
		<script>
			...
			$(document).ready(function () {
				if($("#hdnIsPercentage").val() == true) {
					$("#ddlDiscountTotal").val("true");
				}
				else {
					$("#ddlDiscountTotal").val("false");
				}
				$("#ddlFood").combobox({
				...
				if(parseInt($("#hdnRow").val()) > 0) {
					...
					for(var i=0; i<row; i++) {
						...
						$("#hdnMenuListID" + count).val(d[1].replace("'", ""));
						$("#txtMenuName" + count).val(d[2].replace("'", ""));
						$("#txtQuantity" + count).val(d[3].replace("'", ""));
						$("#txtPrice" + count).val(returnRupiah(d[4].replace("'", "")));
						
						if(d[6].replace("'", "") == true) {
							$("#txtDiscount" + count).val(d[5].replace("'", ""));
							$("#ddlDiscount" + count).val("true");
						}
						else {
							$("#txtDiscount" + count).val(returnRupiah(d[5].replace("'", "")));
							$("#ddlDiscount" + count).val("false");
						}
						$("#record").val(count);
						$("#recordnew").val(count);
					}
