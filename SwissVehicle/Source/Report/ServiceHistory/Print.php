<?php
	if(isset($_POST['ddlMachine'])) {
		...
		$sql = "SELECT
					CONCAT(MM.MachineType, ' - ', MM.MachineCode) AS MachineCode
				FROM
					master_machine MM
				WHERE
		...
		if (ISSET($_GET['searchPhrase']) )
		{
			...
			$where .= " AND ( DATE_FORMAT(TS.TransactionDate, '%d-%m-%Y') LIKE '%".$search."%' OR MM.MachineType LIKE '%".$search."%' OR MI.ItemName LIKE '%".$search."%' OR SD.ItemName LIKE '%".$search."%' OR MM.MachineCode LIKE '%".$search."%' OR SD.Quantity LIKE '%".$search."%' OR SD.Price LIKE '%".$search."%' OR SD.Remarks LIKE '%".$search."%' )";
		}
		class PDF_AutoPrint extends PDF_JavaScript
		{
			...
			function Header() {
				...
				$this->Ln(5);
				//Judul Kolom
				$w = Array(20,20,50,10,20,25,45);
				$this->SetTextColor(0, 0, 0);
				$this->SetFont('Arial', 'B', 14);
				$this->Ln(2);
				$this->Cell(190,0.5,"Laporan Riwayat Servis",0,0,'C');
				$this->Ln(8);
				$this->SetFont('Arial','',9);
				$this->Cell(20,0.5,"Mobil/Mesin",0,0,'L');
				$this->Cell(100,0.5," : ".$GLOBALS['MachineCode'],0,1,'L');
				$this->Ln(5);
				$this->Cell(20,0.5,"Tanggal",0,0,'L');
				$this->Cell(100,0.5," : ".$GLOBALS['date'],0,1,'L');
				$this->Ln(5);
				$header = array('Tanggal', 'Kilometer', 'Barang', 'Qty', 'Harga', 'Total', 'Keterangan');
			...
			function Table() {
				...
				$sql = "SELECT
							...
							IFNULL(MI.ItemName, SD.ItemName) ItemName,
							TS.Kilometer,
							SD.Quantity,
							SD.Price,
						...
						WHERE
							...
							AND MM.MachineID = ".$GLOBALS['MachineID']."
						ORDER BY
							".$GLOBALS['order_by'];
					...
					while($row = mysql_fetch_array($result)) {
						...
						$this->Row(Array($row['TransactionDate'], number_format($row['Kilometer'],0,".",","), $row['ItemName'], $row['Quantity'],  number_format($row['Price'],2,".",","), number_format($row['TotalAmount'],2,".",","), $row['Remarks']));
					}
					$this->Cell(120,5,'Grand Total',1,0,'L');
		...
		$pdf->AddPage();
		$pdf->SetWidths(Array(20,20,50,10,20,25,45));
		$pdf->SetAligns(Array('C', 'R', 'L', 'R', 'R', 'R', 'L'));
		srand(microtime()*1000000);
		$pdf->Table();
		$filename = "Laporan Riwayat Servis  " . $GLOBALS['MachineCode'] . " " . $GLOBALS['date'] . ".pdf";
		//$pdf->AutoPrint(true);
		if($_GET['PrintType'] == "1") {
