								...
								<div class="col-md-3">
									<div class="ui-widget" style="width: 100%;">
										<select name="ddlItem" id="ddlItem" class="form-control-custom" placeholder="Pilih Barang" >
											<option value="" > </option>
											<option value=0 selected>-Pilih Semua Barang-</option>
											<?php
												$sql = "SELECT 
															MI.ItemID,
															MI.ItemName,
		...
		<script>			
			function Preview() {
				...
				if(PassValidate == 0) {
					...
					$("#grid-data").bootgrid({
						...
						url: "Report/Purchase/DataSource.php?ItemID=" + ItemID + "&SupplierID=" + SupplierID + "&txtFromDate=" + txtFromDate + "&txtToDate=" + txtToDate,
						selection: true,
						multiSelect: true,
						rowSelect: true,
						keepSelection: true
			...
			function PrintReport(PrintType) {
				...
				if(PassValidate == 0) {
					...
					form = $("#PostForm");
					form.attr("action", "./Report/Purchase/Print.php?PrintType=" + PrintType + "&searchPhrase=" + searchPhrase + "&sort=" + sortDictionary);
