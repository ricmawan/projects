<?php
	if(isset($_POST['ddlItem'])) {
		...
		if (ISSET($_GET['searchPhrase']) )
		{
			...
			$where .= " AND ( DATE_FORMAT(TS.TransactionDate, '%d-%m-%Y') LIKE '%".$search."%' OR TS.CustomerName LIKE '%".$search."%' OR MI.ItemName LIKE '%".$search."%' OR SD.Quantity LIKE '%".$search."%' OR SD.Price LIKE '%".$search."%' OR SD.Remarks LIKE '%".$search."%' )";
		}
		class PDF_AutoPrint extends PDF_JavaScript
		{
			...
			function Header() {
				...
				$this->SetFont('Arial', 'B', 14);
				$this->Ln(2);
				$this->Cell(190,0.5,"Laporan Penjualan",0,0,'C');
				$this->Ln(8);
				$this->SetFont('Arial','',9);
			...
			function Table() {
				...
				$sql = "SELECT
					...
					while($row = mysql_fetch_array($result)) {
						...
						$this->Row(Array($row['TransactionDate'], $row['CustomerName'], $row['ItemName'], $row['Quantity'],  number_format($row['Price'],2,".",","), number_format($row['TotalAmount'],2,".",","), $row['Remarks']));
					}
		...
		$pdf->Table();
		$filename = "Laporan Penjualan " . $GLOBALS['date'] . ".pdf";
		//$pdf->AutoPrint(true);
		if($_GET['PrintType'] == "1") {
