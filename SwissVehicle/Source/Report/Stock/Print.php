	...
	if(isset($_POST['ddlItem'])) {
		...
		class PDF_AutoPrint extends PDF_JavaScript
		{
			...
			function Header() {
				...
				$this->SetFont('Arial', 'B', 14);
				$this->Ln(2);
				$this->Cell(190,0.5,"Laporan Stok",0,0,'C');
				$this->Ln(8);
				$this->SetFont('Arial','',9);
				...
				$this->Ln(5);
				$header = array('Tanggal', 'Pembelian', 'Pemakaian', 'Stok', 'Keterangan');
				...
				$sql = "SELECT
						...
						FROM
							...
							(
								...
								WHERE
									CAST(TP.TransactionDate AS DATE) < '".$GLOBALS['txtFromDate']."'
									AND PD.ItemID = ".$GLOBALS['ItemID']."
								GROUP BY
									PD.ItemID
							)PD
