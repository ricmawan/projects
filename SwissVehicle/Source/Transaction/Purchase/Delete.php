<?php
	if(isset($_POST['ID'])) {
		...
		for($i=0; $i<count($Data); $i++) {
			try
			{
				$PurchaseID = mysql_real_escape_string($Data[$i]);
				$sql = "DELETE FROM transaction_purchase WHERE PurchaseID = '".$PurchaseID."'";
				if (! $result=mysql_query($sql, $dbh)) {
					throw new Exception($PurchaseID);
				}
				$MessageSuccessDelete .= "$PurchaseID";
			}
			catch (Exception $e)
			{
