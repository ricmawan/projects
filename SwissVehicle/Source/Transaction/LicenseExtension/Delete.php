<?php
	if(isset($_POST['ID'])) {
		...
		for($i=0; $i<count($Data); $i++) {
			try
			{
				$LicenseExtensionID = mysql_real_escape_string($Data[$i]);
				$sql = "DELETE FROM transaction_licenseextension WHERE LicenseExtensionID = '".$LicenseExtensionID."'";
				if (! $result=mysql_query($sql, $dbh)) {
					throw new Exception($LicenseExtensionID);
				}
				$MessageSuccessDelete .= "$LicenseExtensionID";
			}
			catch (Exception $e)
			{
