<?php
	if(isset($_POST['ID'])) {
		...
		for($i=0; $i<count($Data); $i++) {
			try
			{
				$OperationalID = mysql_real_escape_string($Data[$i]);
				$sql = "DELETE FROM transaction_operational WHERE OperationalID = '".$OperationalID."'";
				if (! $result=mysql_query($sql, $dbh)) {
					throw new Exception($OperationalID);
				}
				$MessageSuccessDelete .= "$OperationalID, ";
			}
			catch (Exception $e)
			{
