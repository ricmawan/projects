<?php
	...
	if (ISSET($_REQUEST['sort']) && is_array($_REQUEST['sort']) )
	{
		...
		foreach($_REQUEST['sort'] as $key => $value) {
			...
			else $order_by = "BR.BuyReturnID DESC";
		}
	}
	...
	if (ISSET($_REQUEST['searchPhrase']) )
	{
		...
		$where .= " AND ( RIGHT(BR.BuyReturnNumber, 3) LIKE '%".$search."%' OR BR.Remarks LIKE '%".$search."%' OR MS.SupplierName LIKE '%".$search."%' OR DATE_FORMAT(BR.TransactionDate, '%d-%m-%Y') LIKE '%".$search."%' ) ";
	}
	...
	while ($row = mysql_fetch_array($result)) {
		...
		$row_array['BuyReturnIDNo']= $row['BuyReturnID']."^".$row['BuyReturnNumber'];
