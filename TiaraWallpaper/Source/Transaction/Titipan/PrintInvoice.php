<?php
	...
	if(isset($_POST['hdnTitipanID'])) {
		...
		$sql = "SELECT
					DATE_FORMAT(TIP.TransactionDate, '%d-%m-%Y') TransactionDate,
					TIP.Remarks,
					TIP.CreatedBy
				FROM
		...
		$Data .= $underline1 . "                                                                                                                                         \n" . $underline0;
		$Data .= $condensed1."   Tanggal : " .$TransactionDate ."\n"; //16 
		
		$sql = "SELECT
					TIPD.Quantity,
					TIPD.Price,
					(TIPD.Price * TIPD.Quantity) AS Total,
					TIPD.Remarks
				FROM
		...
		$Data .= "|---------------------------------------------------------------------------------------------------------------------------------------|\n";
		$Data .= "|      Qty      |                        Nama Barang                      |         Harga Satuan         |           Sub Total          |\n";
		$Data .= "|---------------|---------------------------------------------------------|------------------------------|------------------------------|\n";
		
		while($row=mysql_fetch_array($result)) {
			//Qty
			$Data .= "| " . fnSpace(13 - strlen($row['Quantity'])) . $row['Quantity'] . " | ";
			//ItemName
			$Data .= $row['Remarks'] . fnSpace(55 - strlen($row['Remarks'])) . " | ";
			//Harga Satuan
			$Data .= fnSpace(28 - strlen(number_format($row['Price'],2,".",","))) . number_format($row['Price'],2,".",",") . " | ";
			//Total
			$Data .= fnSpace(28 - strlen(number_format($row['Total'],2,".",","))) . number_format($row['Total'],2,".",",") . " |\n";
		...
		//$Data .= "└───────────────┴────────────────────────────────────────────┴────────────┴───────────────────┴───────────────────┴─────────────────────┘\n";
		$Data .= "|---------------------------------------------------------------------------------------------------------------------------------------|\n";
		$Data .= "   Catatan   : " . $Remarks . fnSpace(75 - strlen($Remarks)) ."Total                 Rp. " . fnSpace(19 - strlen(number_format($GrandTotal,2,".",","))) . number_format($GrandTotal,2,".",",") . "\n";
		//$Data .= "|    2,00 m lari  |  MAESTRO 646 XTC                                  |    522  |      155,000.00  |   15.500(10%)  |   100,201,500.00  |\n";
		//$Data .= "   Kredit    : Rp. " . number_format($GrandTotal,2,".",",") . fnSpace(71 - strlen(number_format($GrandTotal,2,".",",")) ) . "PPN (10%)             Rp. " . fnSpace(19 - strlen(number_format($PPN,2,".",","))) . number_format($PPN,2,".",",") . "\n";
