		...
		if($SaleReturnID != 0) {
			...
			if($rowCount > 0) {
				...
				while($row = mysql_fetch_array($result)) {
					...
					array_push($Data, "'".$row['SaleReturnDetailsID']."', '".$row['TypeID']."', '".$row['TypeName']."', '".$row['BatchNumber']."', '".$row['Quantity']."', '".$row['SalePrice']."', '".$row['Discount']."', '".$row['IsPercentage']."', '".$row['BuyPrice']."'");
				}
			...
			$(document).ready(function () {
				...
				if(parseInt($("#hdnRow").val()) > 0) {
					...
					for(var i=0; i<row; i++) {
						...
						$("#hdnBuyPrice" + count).val(d[8].replace("'", ""));
						$("#record").val(count);
						$("#recordnew").val(count);
					}
					Calculate();
